...
- [ ] Reconcile live RLS on `games` / `game_versions` to the declared `is_public`-based policies and column-level anon grants, replacing the prototype's `is_published` policies. Deferred out of Phase 3 deliberately: every generation read and write uses the service-role client and bypasses RLS, so the wrong policies are not on Phase 3's path — but the Studio UI depends on them the moment it reads games through the RLS client.
...
- **Phase 3**: a Zod-invalid `GameSpec` is rejected; an aborted run persists nothing. Token-budget refunds are verified with the quota work in Phase 6, where the budget exists.
