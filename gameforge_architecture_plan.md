...
- **Quota cost**: every `generate`/`patch` run is billed at its terminal state for the tokens it actually spent — a failure or abort is charged for whatever stages had already answered, and a run that never reached a model costs nothing; a repair attempt is billed only when it reached the model. The budget is incremented at the terminal state and never decremented, so there is no refund path at all — see §2.5.
...
- **Public slugs**: auto-generated on first publish as `<slugified-title>-<4-char-suffix>` (e.g. `space-blaster-x7k2`), unique-constrained, and renameable later from Studio. Generation deliberately does **not** happen at creation: every new game is persisted with `public_slug` NULL and `is_public` false, so both are set by the Phase 7 publish action.
...
- **Charging is the whole refund story.** The budget is incremented only at a terminal state and never decremented, and it is charged the tokens the run actually spent. A `/api/generate` or `/api/patch` run is charged whatever the stages that answered consumed — a completed run the full amount, a failure or abort only what it spent before stopping — and every `/api/debug` attempt that reached the model is charged, including a boot-gate tombstone. Because nothing is charged up front, there is nothing to credit back — the earlier design's charge-then-refund path is deleted rather than reimplemented.
- **Enforcement**: `check_run_allowed` runs in every route *before* the run slot is claimed, so a refused caller never takes the slot and never reaches a billable call. `rate_limited` and `quota_exceeded` are separate codes because they have separate remedies — both map to HTTP 429; a check that cannot be answered fails closed as `quota_unavailable` (503). Charging happens in the same `finally`, immediately before the slot is released so the next check sees it, and is best-effort: under-charging is the safe direction for a spend guard.
...
- [x] Implement the quota store (`lib/quota/`): a `QuotaStore` interface with a Postgres implementation, with the limits injected rather than read from the environment. Gate `/api/generate`, `/api/patch` and `/api/debug` on `check_run_allowed` before the run slot is claimed.
- [x] Charge the daily budget at the terminal state. A `generate`/`patch` run is billed the tokens it actually spent — a completed run the full amount, a failure or abort only what it spent before stopping; every `/api/debug` attempt that reached the model is billed, tombstone included. This is why there is no refund path to implement.
...
- [ ] Add the publish action: flip `games.is_public` and, on first publish, assign `public_slug` in the `<slugified-title>-<4-char-suffix>` form under the existing unique `games_public_slug_key` index, renameable afterwards. Owned by the Studio. This is the task earlier sessions deferred from Phase 3/4 into what was then Phase 6 — `/play/[slug]` reads it, but the slug has to be written first — see §2.4.
- [ ] Implement `/play/[slug]` public playable route with OpenGraph tags. This reads through the `games_select_public` policy and the `anon` `select` grant on `games` that Phase 4 already reconciled for exactly this route.
...
- **Phase 3**: a Zod-invalid `GameSpec` is rejected; an aborted run persists nothing. A failed run is charged only for the tokens it actually spent, so the quota side of this phase is verified under Phase 6 instead.
