...
mock.module("@/lib/supabase/admin", () => ({
  createAdminClient: () => ({
    rpc: async (fn: string, args: Record<string, unknown>): Promise<RpcResponse> => {
      calls.push({ fn, args });
      return response;
    },
  }),
...
describe("beginGenerationRun", () => {
  test("returns the claimed run id", async () => {
    response = { data: "run-1", error: null };
  ...
  test("accepts a null game id for a new game", async () => {
    response = { data: "run-2", error: null };

    await beginGenerationRun("user-1", null);

    expect(calls[0].args).toEqual({ p_user_id: "user-1", p_game_id: null });
  });

  test("throws when the database call fails", async () => {
    response = { data: null, error: { message: "deadlock detected", code: "40P01" } };
...
describe("finishGenerationRun", () => {
  ...
  test("logs and swallows a failed release", async () => {
    response = { data: null, error: { message: "connection reset", code: "08006" } };
